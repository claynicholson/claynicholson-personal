---
title: Libraries and Study Spaces
category: resources
summary: The MIT Libraries, borrowing, course reserves, and where people actually study.
sources:
  - https://libraries.mit.edu
last_reviewed: 2026-08
---

## The libraries
- **Hayden Library (Building 14)** — the main library (humanities + science), beautifully renovated, courtyard, café; the default study library.
- **Barker Library (Building 10, under the Great Dome)** — engineering; the reading room under the dome is the iconic study spot.
- **Dewey Library (E53)** — economics, management, social sciences (Sloan side).
- **Rotch Library (7-238)** — architecture, art, and planning.
- **Lewis Music Library (14E)** — scores, recordings, listening rooms.
- **Distinctive Collections** — MIT's archives (historic theses, hack artifacts).

## Borrowing and access
- Borrow with your MIT ID; long loans, easy renewals.
- **Course reserves**: many textbooks are on reserve or licensed as ebooks — check before buying anything.
- **Interlibrary loan / BorrowDirect (Ivy Plus)**: nearly any book or paper on Earth, free, usually within a few business days.
- Massive journal/database subscriptions (IEEE, ACM, Elsevier, JSTOR...) — off campus, log in via Touchstone or VPN. MIT is a leader in open access; many MIT theses are free online (dspace.mit.edu).
- **Harvard borrowing**: MIT students (undergrads included) get access and borrowing at 11 Harvard libraries — apply online for an HCL Special Borrower card and pick up the ID at Widener. Details at libraries.mit.edu/borrow.

## Study spaces people actually use
- Hayden (social-quiet mix), Barker reading room (silent, majestic), the Student Center (W20) study floors (24/7 areas), Stata's public areas, department lounges, and empty classrooms at night (building access with ID).
- During finals, libraries extend hours and dorms/desks publish quiet-hours.
- Booking group study rooms: via the libraries' room-reservation system.

## Beyond books
Workshops on citation management (Zotero), data management, GIS help, 3D scanning; librarians do research consultations for papers and theses — genuinely useful and free.
